...
TaskHandle_t LCD_Serial_Handler;
...
/*Internal Variables*/
String received_msg;
String received_msg1;
String received_msg2;
int previous_temp;
int previous_hum;
bool loadingLCD;
bool logging;
...
void registry_update(registry* x){  //Registry update function that takes in address of the struct variable
  ...
  received_msg1=Serial2.readString();
  received_msg1.trim();
  x->animal_type.push_back(received_msg1.toInt());
  ...
  received_msg2.trim();
  x->ID.push_back(received_msg2.toInt());
  Serial2.print("received");
  ...
  switch(reg){
    ...
    case SalesDelivery:
        if (received_msg1.toInt() == 0){ //if pig
        json6.set(salesPig_ID.c_str(), String(received_msg2)); //String(Pig ID)
        json6.set(timePath6, "timestamp");
        Firebase.RTDB.pushJSON(&fbdo, salesPig_Path.c_str(), &json6);
        }
        else if (received_msg1.toInt() == 1){ //if chicken
        json3.set(salesChic_ID.c_str(), String(received_msg2)); //String(Chicken ID)
    ...
    case Recovery:
        if (received_msg1.toInt() == 0){ //if pig
        json7.set(recoveryPig_ID.c_str(), String(received_msg2)); //String(Pig ID)
        json7.set(timePath7, "timestamp");
        Firebase.RTDB.pushJSON(&fbdo, recoveryPig_Path.c_str(), &json7);
        }
        else if (received_msg1.toInt() == 1){ //if chicken
        json4.set(recoveryChic_ID.c_str(), String(received_msg2)); //String(Chicken ID)
    ...
    case Deceased:
        if (received_msg1.toInt() == 0){ //if pig
        json8.set(deceasedPig_ID.c_str(), String(received_msg2)); //String(Pig ID)
        json8.set(timePath8, "timestamp");
        Firebase.RTDB.pushJSON(&fbdo, deceasedPig_Path.c_str(), &json8);
        }
        else if (received_msg1.toInt() == 1){ //if chicken
        json5.set(deceasedChic_ID.c_str(), String(received_msg2)); //String(Chicken ID)
